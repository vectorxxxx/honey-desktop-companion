...
public sealed class WhiteJadeSpiderScene : IRenderScene
{
        ...
        float offsetY)
    {
        ...
        var layout = SpiderLayout.Create(bounds.Width, bounds.Height, safe.Scale);
        canvas.Save();
        canvas.Translate(offsetX, offsetY);
        DrawParticles(canvas, layout, safe);
        _webScene.Draw(canvas, new SKRect(0, 0, bounds.Width, bounds.Height), safe);
        DrawLegs(canvas, layout, safe);
        DrawBody(canvas, layout, safe);
        DrawEyes(canvas, layout, safe);
        canvas.Restore();
    }

    private static void DrawParticles(SKCanvas canvas, SpiderLayout layout, RenderSnapshot snapshot)
    {
        ...
        var span = layout.Abdomen.Height * 2.2f;
        for (var index = 0; index < 14; index++)
        {
            var seed = index * 12.9898;
            var phase = snapshot.AnimationTime * (berserk ? 1.9 : 0.65) + seed;
            var x = layout.Center.X + MathF.Sin((float)(seed * 0.73)) * span;
            var y = layout.Center.Y + MathF.Cos((float)(seed * 0.41 + phase)) * span * 0.8f;
    ...
    private static void DrawLegs(SKCanvas canvas, SpiderLayout layout, RenderSnapshot snapshot)
    {
        var berserk = snapshot.Mode == PetMode.Berserk;
        var cadence = berserk ? 5.4 : 3.1;
        var moodAmplitude = snapshot.Mood switch
        {
        ...
        for (var index = 0; index < layout.Legs.Count; index++)
        {
            ...
            var phase = snapshot.AnimationTime * cadence + index * Math.PI / 2;
            var lift = MathF.Sin((float)phase) * leg.Width * moodAmplitude;
            var knee = new SKPoint(leg.Knee.X, leg.Knee.Y + lift);
            var tip = new SKPoint(leg.Tip.X, leg.Tip.Y - lift * 0.35f);
            ...
            canvas.DrawCircle(knee, leg.Width * 0.43f, jade);
        }
    }

    private static void DrawBody(SKCanvas canvas, SpiderLayout layout, RenderSnapshot snapshot)
    {
        var berserk = snapshot.Mode == PetMode.Berserk;
        ...
        DrawJadeEllipse(canvas, layout.Abdomen, edge, middle, highlight);
        DrawJadeEllipse(canvas, layout.Head, edge, middle, highlight);

        using var vein = new SKPaint
        {
            IsAntialias = true,
            Style = SKPaintStyle.Stroke,
            StrokeWidth = Math.Max(1, layout.Abdomen.Width * 0.025f),
            Color = berserk
        ...
        veinBuilder.MoveTo(layout.Abdomen.MidX, layout.Abdomen.Top + layout.Abdomen.Height * 0.18f);
        veinBuilder.CubicTo(
            layout.Abdomen.Left + layout.Abdomen.Width * 0.28f,
            layout.Abdomen.MidY,
            layout.Abdomen.Right - layout.Abdomen.Width * 0.2f,
            layout.Abdomen.MidY,
            layout.Abdomen.MidX,
            layout.Abdomen.Bottom - layout.Abdomen.Height * 0.15f);
    ...
    private static void DrawEyes(SKCanvas canvas, SpiderLayout layout, RenderSnapshot snapshot)
    {
        var berserk = snapshot.Mode == PetMode.Berserk;
        ...
        for (var row = 0; row < 2; row++)
        {
            for (var pair = 0; pair < 2; pair++)
            {
                ...
                {
                    var x = layout.Head.MidX + side * unit * (pair == 0 ? 0.14f : 0.31f);
                    var y = layout.Head.Top + layout.Head.Height * (row == 0 ? 0.42f : 0.67f);
